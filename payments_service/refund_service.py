...
from payments_service.interfaces.refund_service_interface import \
...
class RefundService(RefundServiceInterface):
    ...
    async def _get_updated_data(user_id: str, payment_id: str):
        return {
            'filter': {
                'user_id': user_id,
                'youkassa_payment_id': payment_id},
            'update': {
                '$set': {
                    'status_payment': PaymentStatus.CANCELED.value,
    ...
    async def refund_money(
            self, user_id: str, payment_id: str, amount: float, currency: str,
            payment_collection
    ):
        refund_object = {
            "amount": {
                "value": str(amount), "currency": currency.upper()
            },
        ...
        await payment_collection.update_one(
            **await self._get_updated_data(user_id, payment_id)
        )
        return refund
