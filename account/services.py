...
class AccountService:
    def __init__(self, repository: AccountRepositoryInterface, token_service: TokenServiceInterface,
    ...
    async def _create_token_data(self, username: str, token_type: TokenType, exp_time: int):
        return await self._token_service.encode_token(
            username=username, token_type=token_type, secret_key=self._settings.api_key,
    ...
    async def _authenticate(self, username: str, password: str):
        if not (user := await self._repository.get_user_by(username=username)) \
                or not await self._password_service.verify_passwords(plain_password=password,
    ...
    async def login(self, username: str, password: str):
        ...
        access_token = await self._create_token_data(
            username=user['username'], token_type=TokenType.ACCESS_TOKEN, exp_time=self._settings.exp_time)
        refresh_token = await self._create_token_data(
            username=user['username'], token_type=TokenType.REFRESH_TOKEN, exp_time=10)
