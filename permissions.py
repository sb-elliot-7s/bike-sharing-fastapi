...
class Permission:
    ...
    def __init__(self, token_service: TokenServiceInterface):
        ...
        self._secret_key, self._algorithm = get_configs().api_key, get_configs().algorithm

    async def _decode_token(self, token: str) -> str:
        payload: dict = await self._token_service.decode_access_token(
            access_token=token, secret_key=self._secret_key, algorithm=self._algorithm
        )
    ...
    async def __call__(self, account_collection=Depends(get_account_collection),
        ...
        user = await self._find_user(account_collection=account_collection, _filter={'username': username})
        return AccountSchema(**user)

    async def get_admin_user(self, account_collection=Depends(get_account_collection),
                             token: str = Depends(OAUTH2_SCHEME)):
        username = await self._decode_token(token=token)
        user = await self._find_user(account_collection=account_collection,
