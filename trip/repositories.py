...
class TripRepository(TripRepositoryInterface):
    ...
    async def get_detail_trip(self, trip_id: str, user_id: str):
        return await self._trip_collection.find_one(filter={'_id': ObjectId(trip_id), 'user_id': user_id})

    async def create_trip(self, bike_id: str, user_id: str, weather_service: WeatherNetworkService):
        async with await client.start_session() as session:
            async with session.start_transaction():
                bike = await self._bike_collection.find_one(filter={'_id': ObjectId(bike_id)})
                station = await CommonStationService().get_and_update_station(
                    station_collection=self._station_collection, station_id=bike['station_id'],
                    _update={
                        '$inc': {'available_count_of_bicycles': -1},
                        '$pull': {'bicycles': {'_id': ObjectId(bike_id)}}}
                )
                await self._bike_collection \
                    .update_one(filter={'_id': ObjectId(bike_id)}, update={'$set': {'station_id': None}})
                weather = await CheckWeatherService().data(station['address']['city'], weather_service)
                document = await PrepareDocumentService() \
                    .prepare_start_trip_document(bike_id=bike_id, bike=bike, user_id=user_id, weather=weather)
        ...
        async with await client.start_session() as session:
            async with session.start_transaction():
                trip = await CommonTripService() \
                    .get_trip(self._trip_collection, user_id=user_id, payment_status=PaymentStatus.UNPAID)
                bike = await CommonBikeService() \
                    .bike_update(self._bike_collection, bike_id=trip['bike_id'], station_id=station_id)
                end_time, travel_time = CalculatesService().calculate_datetime(start_time=trip['start_time'])
                total_amount = CalculatesService().calculate_total_amount(travel_time, trip['price_bike'])
                # from gps -> points
                points = []
                station = await CommonStationService().get_and_update_station(
                    station_collection=self._station_collection, station_id=station_id,
                    _update={
                        '$inc': {'available_count_of_bicycles': 1},
                        '$push': {'bicycles': bike}}
                )
                ...
                return await self._trip_collection.find_one_and_update(
                    filter={'user_id': user_id, 'payment_status': PaymentStatus.UNPAID.value},
