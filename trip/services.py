...
class TripService:
    ...
    async def create_trip(self, bike_id: str, user_id: str, weather_service: WeatherNetworkService):
        return await self.repository \
            .create_trip(bike_id=bike_id, user_id=user_id, weather_service=weather_service)

    async def finish_trip(self, station_id: str, user_id: str, weather_service: WeatherNetworkService):
        return await self.repository \
            .finish_trip(station_id=station_id, user_id=user_id, weather_service=weather_service)

    async def get_trip(self, trip_id: str, user_id: str):
        return await self.repository.get_detail_trip(trip_id=trip_id, user_id=user_id)
